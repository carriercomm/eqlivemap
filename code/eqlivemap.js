  ...
  drawMap = function() {
    ...
    if(typeof canvas == "object" && canvas && typeof canvas.getContext == "function"){
      ...
      var width = ranges.x.max - ranges.x.min,
          ...
          scaleX = canvas.width / width *.5,
          scaleY = canvas.height / height * .5
          console.log(ranges)
        ...
        _.each(mapData.split("\n"), function(data){
          if(data.match(/L /)){
           // console.log("line")
            ...
            var figure = {map: realMapName, type: "line", y1: parseInt(points[0]), x1: parseInt(points[1]), z1: parseInt(points[2]), y2: parseInt(points[3]), x2: parseInt(points[4]), z2: parseInt(points[5]), r: parseInt(points[6]), g: parseInt(points[7]), b: parseInt(points[8])}
            ...
            var figure = {map: realMapName, type: "label", y: parseInt(points[0]), x: parseInt(points[1]), z: parseInt(points[2]), r: parseInt(points[3]), g: parseInt(points[4]), b: parseInt(points[5]), font_size: parseInt(points[6]), label: points[7] }
            Figures.insert(figure, function(){})
          }
        })
      })
